...
NS_BEGIN(Client)
class CMoriblinSword_Follow : public IPawnState
{
private:
...
public:
	...
	{
		return ENUM_TO_UINT(CMonster_MoriblinSword::STATE_ID::FOLLOW);
	}
	...
	virtual void OnStateStay(class CPawnObject* _pActor, _float _fTimeDelta) override
	{
		...
		if (fDist <= 2.f)
		{
			...
			if (fRand < 80.f)
				_pActor->RequestToChangeState(ENUM_TO_UINT(CMonster_MoriblinSword::STATE_ID::ATTACK));
			else
				_pActor->RequestToChangeState(ENUM_TO_UINT(CMonster_MoriblinSword::STATE_ID::GUARD));
		}
		else if (fDist > 8.f)
		{
			_pActor->RequestToChangeState(ENUM_TO_UINT(CMonster_MoriblinSword::STATE_ID::IDLE));
