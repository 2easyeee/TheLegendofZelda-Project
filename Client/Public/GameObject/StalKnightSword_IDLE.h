...
NS_BEGIN(Client)
class CStalKnightSword_IDLE : public IPawnState
{
private:
...
public:
	...
	{
		return ENUM_TO_UINT(CMonster_StalKnightSword::STATE_ID::IDLE);
	}
	...
	virtual void OnStateStay(class CPawnObject* _pActor, _float _fTimeDelta) override
	{
		...
		if (fDist < 8.f)
		{
			_pActor->RequestToChangeState(ENUM_TO_UINT(CMonster_StalKnightSword::STATE_ID::FOLLOW));
			return;
		}
		...
		if (fRand < 60.f)
			_pActor->RequestToChangeState(ENUM_TO_UINT(CMonster_StalKnightSword::STATE_ID::WALK));
	}
