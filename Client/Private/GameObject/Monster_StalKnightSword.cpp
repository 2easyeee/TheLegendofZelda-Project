...
#include "GameObject/StalKnightSword_Attack.h"
#include "GameObject/StalKnightSword_Damage.h"
#include "GameObject/StalKnightSword_Dead.h"
#include "GameObject/StalKnightSword_Follow.h"
#include "GameObject/StalKnightSword_Guard.h"
#include "GameObject/StalKnightSword_IDLE.h"
#include "GameObject/StalKnightSword_Piyo.h"
#include "GameObject/StalKnightSword_Walk.h"

CMonster_StalKnightSword::CMonster_StalKnightSword(ID3D11Device* _pDevice, ID3D11DeviceContext* _pDeviceContext)
	: CPawnObject{ _pDevice, _pDeviceContext }
{
}

CMonster_StalKnightSword::CMonster_StalKnightSword(const CMonster_StalKnightSword& _Prototype)
	: CPawnObject(_Prototype)
{
}

HRESULT CMonster_StalKnightSword::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CMonster_StalKnightSword::Initialize(void* _pArg)
{
	/* 1. Pawn Object */
	...
	/* 5. Init State */
	Register_State(new CStalKnightSword_IDLE());
	Register_State(new CStalKnightSword_Walk());
	Register_State(new CStalKnightSword_Follow());
	Register_State(new CStalKnightSword_Piyo());
	Register_State(new CStalKnightSword_Attack());
	Register_State(new CStalKnightSword_Guard());
	Register_State(new CStalKnightSword_Damage());
	Register_State(new CStalKnightSword_Dead());
...
void CMonster_StalKnightSword::Priority_Update(_float _fTimeDelta)
{
	if (!m_bActive)
		return;
...
void CMonster_StalKnightSword::Update(_float _fTimeDelta)
{
	if (!m_bActive)
		return;
...
void CMonster_StalKnightSword::Late_Update(_float _fTimeDelta)
{
	if (!m_bActive)
		return;
...
HRESULT CMonster_StalKnightSword::Render()
{
	if (!m_bActive)
		return S_OK;
...
void CMonster_StalKnightSword::Collider_BodyEnable(_bool _bActive)
{
	if (m_pBodyCollider)
		m_pBodyCollider->Set_Active(_bActive);
}

void CMonster_StalKnightSword::Collider_WeaponEnable(_bool _bActive)
{
	if (m_pWeaponCollider)
		m_pWeaponCollider->Set_Active(_bActive);
}

void CMonster_StalKnightSword::Collider_ShieldEnable(_bool _bActive)
{
	if (m_pShieldCollider)
		m_pShieldCollider->Set_Active(_bActive);
}

void CMonster_StalKnightSword::Collider_SensorEnable(_bool _bActive)
{
	if (m_pSensorCollider)
		m_pSensorCollider->Set_Active(_bActive);
}

void CMonster_StalKnightSword::OnCollisionEnter(CCollider* _pSrc, CCollider* _Dst)
{
	...
	if (_pSrc->Get_Group() == GROUP::SHIELD_MONSTER)
	{
		if (_Dst->Get_Group() == GROUP::WEAPON_PLAYER)
		{
			// KnockBack
			CTransform* pTargetTransform = static_cast<CTransform*>(_Dst->Get_Owner()->Get_Component(TEXT("Com_Transform")));
			_vector vWeaponPos = pTargetTransform->Get_State(STATE::POSITION);
			Start_KnockBack(vWeaponPos, 5.f, 0.5f);

			/* SFX */
			m_pGameInstance->Play_Sound(L"Player_Shield_Guard_0.wav", SOUND::EFFECT);
		}
	}
}

void CMonster_StalKnightSword::OnCollisionStay(CCollider* _pSrc, CCollider* _Dst)
{
}

void CMonster_StalKnightSword::OnCollisionExit(CCollider* _pSrc, CCollider* _Dst)
{
}

_float CMonster_StalKnightSword::Compute_Distance_To_Target()
{
	if (!m_pTarget)
		return FLT_MAX;
...
HRESULT CMonster_StalKnightSword::Ready_Collider()
{
	/* Collider (Body) */
...
HRESULT CMonster_StalKnightSword::Attach_Socket()
{
	if (!m_pModelCom)
		return S_FALSE;
...
HRESULT CMonster_StalKnightSword::Ready_Navigation()
{
...
void CMonster_StalKnightSword::Take_Damage(_float _fDamage, _vector _vHitPos)
{
	if (m_fHP < 0.f)
		return;
...
CMonster_StalKnightSword* CMonster_StalKnightSword::Create(ID3D11Device* _pDevice, ID3D11DeviceContext* _pDeviceContext)
{
	CMonster_StalKnightSword* pInstance = new CMonster_StalKnightSword(_pDevice, _pDeviceContext);
	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CMonster_StalKnightSword");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CMonster_StalKnightSword::Clone(void* _pArg)
{
	CMonster_StalKnightSword* pInstance = new CMonster_StalKnightSword(*this);
	if (FAILED(pInstance->Initialize(_pArg)))
	{
		MSG_BOX("Failed to Cloned : CMonster_StalKnightSword");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CMonster_StalKnightSword::Free()
{
	__super::Free();
